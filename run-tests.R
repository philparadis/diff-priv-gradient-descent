...
### TEST FUNCTION: gen.synthetic.data.binary.labels

test.gen.synthetic.data <- function()
{
  ### UNIT TESTS: gen.synthetic.data.continous.labels
  ...
  ### UNIT TESTS: gen.synthetic.data.binary.labels
  ### TODO: Should be in its own file
  n <- 2
  theta <- c(1, -2)
  ...
  n <- 2
  theta <- c(1, -2)
  data <- gen.synthetic.data.continous.labels(2, theta = c(1, -2), seed = 0)
  data
  sigmoid.loss.fn(theta, data)
  model <- dgd(data)
  model$best$theta
...
### TEST FUNCTION: gen.synthetic.data.continous.labels
test.continuous.labels <- function()
{
